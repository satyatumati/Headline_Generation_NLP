...
args_ = sys.argv
total_lines = int(args_[1])
total_iter = int(args_[2])
rnn_layers = 2
embedtype = 'glove'

FN = 'train-weights-lines_%s-iter_%s-layers_%s'%(total_lines,total_iter,rnn_layers) # name of the file the model weights will be saved to (train.hdf5)
FN0 = 'json_news-%s-%s-vocab-embedding'%(total_lines,embedtype)
FN1 = 'train-log-lines_%s-iter_%s-layers_%s'%(total_lines,total_iter,rnn_layers) # name of the file the model weights to load from (if starting with a pre-existing model)



reload(logging)
logging.basicConfig(filename=FN1,level=logging.INFO)
...
with open('pickles/%s.data.pickle'%FN0, 'rb') as fp:
    X, Y = pickle.load(fp)
...
for iteration in range(501):
    ...
    with open('pickles/%s.history.pickle'%FN,'wb') as fp:
        pickle.dump(history,fp,-1)
    if(iteration%100==0):
        base_model.save_weights('pickles/%s_iter%d.hdf5'%(FN,iteration), overwrite=True)
