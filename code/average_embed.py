...
if __name__ == "__main__":
  ...
  nr = 5000
  bno = 0
  with open("elmo_embedding_nr-"+str(nr)+"_bsiz-100.txt", 'r') as f:
    data = []
    for line in f:
  ...
  # save_emb_to_file(embed_dict, "elmo_embedding_news50000.txt")
  ...
  for emb, w in zip(X, embed_dict):
    reduce_dict[w] = emb 
  save_emb_to_file(reduce_dict, "elmo_"+str(nr)+"_dim300.txt")
